...
for i in range(1,112,2):
	...
	def all_fourier_calculations(total_rows,U,V,W,M):
		...
		for j in range(0,total_rows):
			...
			for k in range(1, int((M - 1) / 2) + 1):
				...
				partial_sum_U = partial_sum_U + ak_list_U[k - 1] * np.cos(2 * math.pi * (j / total_rows) * k) + bk_list_U[k - 1] * np.sin(2 * math.pi * (j / total_rows) * k)
				partial_sum_V = partial_sum_V + ak_list_V[k - 1] * np.cos(2 * math.pi * (j / total_rows) * k) + bk_list_V[k - 1] * np.sin(2 * math.pi * (j / total_rows) * k)
				partial_sum_W = partial_sum_W + ak_list_W[k - 1] * np.cos(2 * math.pi * (j / total_rows) * k) + bk_list_W[k - 1] * np.sin(2 * math.pi * (j / total_rows) * k)
	...
	U_prime_U_prime_Average, V_prime_V_prime_Average, W_prime_W_prime_Average = all_fourier_calculations(total_rows,U_prime_U_prime,V_prime_V_prime,W_prime_W_prime,M)
	U_prime_V_prime_Average, V_prime_W_prime_Average, W_prime_U_prime_Average = all_fourier_calculations(total_rows,U_prime_V_prime,V_prime_W_prime,W_prime_U_prime,M)
	...
	# plt.subplot(311)
	# plt.scatter(Timezero,U,color='r',marker="o")
	# plt.plot(Timezero,U,color='r', label='U',marker="o")
	# plt.scatter(Timezero,U_j_array,marker="+")
	# plt.plot(Timezero,U_j_array, label='U_avg (Fourier_M)',marker="+")
	# plt.legend(loc='upper right')
	# plt.show(block = True)

	# plt.subplot(312)
	# plt.scatter(Timezero,V,color='r',marker="o")
	# plt.plot(Timezero,V,color='r', label='V',marker="o")
	# plt.scatter(Timezero,V_j_array,marker="+")
	# plt.plot(Timezero,V_j_array, label='V_avg (Fourier_M)',marker="+")
	# plt.legend(loc='best')
	# plt.show(block = True)
	 # #https://matplotlib.org/devdocs/api/_as_gen/matplotlib.pyplot.subplots.html
	# plt.subplot(313)
	# plt.scatter(Timezero,W,color='r',marker="o")
	# plt.plot(Timezero,W,color='r', label='W',marker="o")
	# plt.scatter(Timezero,W_j_array,marker="+")
	# plt.plot(Timezero,W_j_array, label='W_avg (Fourier_M)',marker="+")
	# plt.legend(loc='best')
	# plt.savefig('Fourier_M.png', bbox_inches='tight')
	...
	print((type(Timezero), type(U)))
	#plt.scatter(x, y, s=area, c=colors, alpha=0.5)
	# plt.scatter(Timezero,U)
	# plt.show(block = True)

	end_time = time.time()
	print(("\n\nElapsed time was %f seconds" % (end_time - start_time)))
end_time_overall = time.time()

print(("\n\nOverall Elapsed time  was %f seconds" % (end_time_overall - start_time_overall)))